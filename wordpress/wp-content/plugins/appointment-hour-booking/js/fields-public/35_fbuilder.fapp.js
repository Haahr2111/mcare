		    	        ...
		    	        { 
		    	            ...
		    	            if (compactUsedSlots[i].serviceindex==me.service_selected)
		    	            {
		    	                ...
		    	                if (compactUsedSlots[i].quantity>=capacity_service)
		    	                    compactUsedSlots[i].html = '<div class="htmlUsed '+((typeof compactUsedSlots[i].s !== 'undefined')?compactUsedSlots[i].s.replace(/ /g,"").toLowerCase():"")+'"><a '+((typeof compactUsedSlots[i].e !== 'undefined')?"title=\""+compactUsedSlots[i].e+"\"":"")+'>'+formatString(compactUsedSlots[i],false,me.tz)+'</a>'+((typeof compactUsedSlots[i].e !== 'undefined')?"<div class=\"ahbmoreinfo\">"+compactUsedSlots[i].e+"</div>":"")+'</div>';
		    	                if (me.bSlotsCheckbox && me.usedSlotsCheckbox)
		    		...
		    		var html = "";
		  			for (var i=0;i<arr.length;i++)
		  			{
		  			  	...
		  			  	{ 
		  			  	    ...
		  			  	    {
		  			  	        html = "<div><a href=\"\" d=\""+arr[i].day+"\" h1=\""+Math.floor((st)/60)+"\" m1=\""+((st)%60)+"\" h2=\""+Math.floor((st+duration)/60)+"\" m2=\""+((st+duration)%60)+"\">"+formatString({st:st,et:st+duration},false,me.tz)+"<span class=\"ahb_slot_availability\"><span class=\"p\">ahbslotavailabilityP</span><span class=\"t\">ahbslotavailabilityT</span></span></a></div>";
		  			...
		  			$( '#field' + me.form_identifier + '-' + me.index + ' .usedSlots'+me.name+ ' a.cancel').off("click").on("click", function() 
		  			{
